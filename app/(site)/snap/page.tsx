...
export default async function SnapPage({ searchParams }: { searchParams: Promise<{ brand?: string; item?: string }> }) {
  ...
  return <main className="mx-auto max-w-6xl px-4 py-5 sm:py-10">
    <header className="mb-5 max-w-2xl sm:mb-8"><p className="mb-2 text-xs font-black tracking-[0.24em] text-accent">EDITORIAL SNAP</p><h1 className="mb-3 text-3xl font-black">編集部スナップ</h1><p className="text-sm leading-relaxed text-muted-foreground">編集部が撮影・掲載許可を確認した、街のリアルな着こなしと愛用品を届けます。</p></header>
    <form className="mb-5 grid gap-3 rounded-xl border border-border bg-secondary/30 p-4 sm:mb-8 sm:grid-cols-[1fr_1fr_auto]">
    ...
    {articles.length > 0 ? <div className="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2 sm:gap-y-8 lg:grid-cols-3">{articles.map((article, index) => <ArticleCard key={article.id} article={article} priority={index < 3} />)}</div> : <div className="rounded-xl border border-dashed border-border p-10 text-sm text-muted-foreground">該当するスナップはまだありません。</div>}
  </main>
}
