...
import type {
  Article, ContentType, InformationStatus,
  Category,
  AffiliateLink,
  GalleryImage,
  OfficialLink,
...
export function EditArticleForm({ article }: { article: Article }) {
  ...
  const [snapDisplayName, setSnapDisplayName] = useState(article.snapProfile?.displayName ?? "")
  const [snapAgeGroup, setSnapAgeGroup] = useState(article.snapProfile?.ageGroup ?? "")
  const [snapOccupation, setSnapOccupation] = useState(article.snapProfile?.occupation ?? "")
  const [snapLocation, setSnapLocation] = useState(article.snapProfile?.location ?? "")
  const [snapStylePoint, setSnapStylePoint] = useState(article.snapProfile?.stylePoint ?? "")
  const [snapInstagramUrl, setSnapInstagramUrl] = useState(article.snapProfile?.instagramUrl ?? "")
  const [snapConsentConfirmed, setSnapConsentConfirmed] = useState(Boolean(article.snapProfile?.consentConfirmed))
  const [snapItems, setSnapItems] = useState<SnapItem[]>(article.snapProfile?.items ?? [])
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const res = await fetch(`/api/admin/articles/${article.id}`, {
      ...
      body: JSON.stringify({
        ...
        ...(contentType === "SNAP"
          ? { snapProfile: { displayName: snapDisplayName, ageGroup: snapAgeGroup, occupation: snapOccupation, location: snapLocation, stylePoint: snapStylePoint, instagramUrl: snapInstagramUrl, consentConfirmed: snapConsentConfirmed, consentConfirmedAt: article.snapProfile?.consentConfirmedAt ?? new Date().toISOString(), items: snapItems } }
...
function SnapEditor({ displayName, setDisplayName, ageGroup, setAgeGroup, occupation, setOccupation, location, setLocation, stylePoint, setStylePoint, instagramUrl, setInstagramUrl, consentConfirmed, setConsentConfirmed, items, setItems }: {
  ...
  consentConfirmed: boolean; setConsentConfirmed: (v: boolean) => void; items: SnapItem[]; setItems: (v: SnapItem[]) => void
}) {
  const updateItem = (index: number, patch: Partial<SnapItem>) => setItems(items.map((item, i) => i === index ? { ...item, ...patch } : item))
  return <section className="space-y-4 rounded-xl border border-accent/40 bg-accent/5 p-4">
    <div><p className="text-sm font-black">編集部スナップ情報</p><p className="text-xs text-muted-foreground">カバー画像は全身写真、追加画像はディテール写真です。</p></div>
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <Field label="年代"><input className={inputClass} value={ageGroup} onChange={(e) => setAgeGroup(e.target.value)} /></Field>
      ...
      <Field label="Instagram URL"><input className={inputClass} value={instagramUrl} onChange={(e) => setInstagramUrl(e.target.value)} /></Field>
    ...
    <div className="space-y-2"><p className="text-xs font-semibold">着用アイテム</p>{items.map((item, index) => <div key={index} className="grid gap-2 rounded-lg border border-border p-3 sm:grid-cols-4">
      <input className={inputClass} value={item.category ?? ""} onChange={(e) => updateItem(index, { category: e.target.value })} placeholder="部位" />
    ...
    <label className="flex items-center gap-2 text-sm font-bold"><input type="checkbox" checked={consentConfirmed} onChange={(e) => setConsentConfirmed(e.target.checked)} />撮影・掲載・SNSリンクの掲載同意を確認済み</label>
  </section>
}
