 ...
 */
export function PurchaseChannelsSection({ channels }: { channels: PurchaseChannelInfo[] }) {
  ...
  return (
    <div className="my-8 overflow-hidden rounded-xl border border-border">
      ...
      {official.length > 0 && <ChannelGroup label="公式・正規販売店" channels={official} />}
      {secondary.length > 0 && <ChannelGroup label="セレクト店・二次流通" channels={secondary} />}
    </div>
  )
}

function ChannelGroup({ label, channels }: { label: string; channels: PurchaseChannelInfo[] }) {
  return (
    <div>
      ...
      <div className="divide-y divide-border">
        {channels.map((c, i) => (
          <div key={i} className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5 px-4 py-3">
            {c.url && isSafeExternalUrl(c.url) ? (
              <a href={c.url} target="_blank" rel="noopener noreferrer" className="font-bold text-foreground underline decoration-accent decoration-2 underline-offset-2">
                {c.retailerName}
              </a>
            ) : (
