...
`/api/cron/daily-auto-publish`を毎時15分・45分に実行し、各2時間枠で累計3件に達するまで再試行する。自動公開できるのは、
公式確定情報、公式リンク、検証可能な出典、公式と照合できる画像または編集部制作画像、具体的な商品検索語が
すべて揃う通常記事に加え、出典と画像を照合でき、記事内で未確認情報だと明示するGoss!p記事を対象とする。
リーク、PR・タイアップ、SNAP・人物情報、動画、画像権利が不明な記事は下書きに残して人間レビューへ回す。
