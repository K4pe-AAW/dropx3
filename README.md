...
サイドバーにはプロテイン、ボディケアに加えて「食事・宅食」の独立PR枠を設ける。
食事枠は`SIDEBAR_MEAL_AFFILIATE_PROMO`で一元管理し、現在は提携済みの楽天市場検索リンクを使用する。
nosh等の専用ASP案件と提携した場合は、この設定を発行済みの公式リンク・素材へ差し替える。

## コンテンツの2つの入り口

### 編集部コラム／おすすめ品

管理画面の記事タイプで `COLUMN` または `PICKS` を選ぶと、執筆者名・連載名・PR指定を設定できる。
公開後はそれぞれ `/column` と `/picks` に自動掲載され、トップページにも最新3件の専用枠が表示される。
読了・購入クリックは既存のGA4イベントへ `content_type=COLUMN|PICKS` を付けて集計する。

`SNAP` を選ぶと編集部スナップとして、名前・年代・職業・撮影場所・着用品・今日のポイント・
許可済みInstagram URL・撮影／掲載同意を入力できる。公開後は `/snap` に掲載され、ブランド／
アイテムで絞り込める。同じ全身画像、または同じ人物名＋撮影場所の重複公開は拒否する。
視聴者投稿を自動公開する機能は持たず、編集部が確認した内容だけを公開する。

タイアップ・提供記事は「PR表記」を有効にする。アフィリエイトリンクが1件でもある記事は、従来どおり
この指定に関係なく自動でプロモーション表記が出る。
...
[vercel.json](vercel.json) に `GET /api/cron/collect` を1時間おきに実行する
...
`/api/cron/daily-auto-publish`を毎時15分・45分に実行し、各2時間枠で最低2件・最大5件を公開する。最低2件に届かない場合は同じ枠内で再試行し、良質な候補が追加された場合は最大5件まで公開できる。
公開候補の選び方は2026-09-04までの運用へ戻し、下書きを情報区分や媒体名で一律除外しない。
本文、参照可能な出典または公式リンク、カバー画像、具体的な商品検索語が揃った下書きからランダムに公開を試す。
YouTubeは4時間（6記事）につき最大2件まで先行選定し、残りを通常記事にする。
公開条件を満たすYouTube候補がない場合は無理に混ぜず、通常記事で公開枠を埋める。
女性向け単独記事は6記事中最大1件、FASHIONSNAP由来は12記事中最大1件に抑える。
Goss!p・リークは未確認表示を必ず残し、断定情報としては扱わない。
状態ファイルで同じJSTの2時間枠の公開数を記録し、5件に達した後の重複実行を防止する。

#### 公開済み記事の自動再確認

`/api/cron/recheck-published` を毎日7:30（JST）に実行する。過去120日以内の記事から、記事に保存済みの
確認済み公式商品ページまたは楽天の商品詳細ページを持つものを1回最大4件再確認する。価格・発売日・本文に
更新があれば既存URLのまま更新し、Goss!pは商品実在の根拠を確認できた場合だけREPORTへ変更する。リークは
解除しない。検索結果や推測した公式ドメインは使わず、カバー画像・ギャラリー画像の追加・削除も行わない。
...
| `/admin/operations` | 公開数・失敗理由・下書き滞留・媒体配分・再確認結果（要ログイン） |
| `/admin/monetization` | 購入導線・収益改善候補（要ログイン） |

## 既知の制約・今後の課題

- **Blobの同時更新**: 記事・下書き・収集状態はVercel Blob上のJSONを単位として読み書きする。現在の
  規模では運用できているが、同時編集やデータ量が増えた段階でDB移行を検討する。
- **人気ランキング**: 直近7日間の閲覧・読了・購入クリックを匿名集計して順位付けする。データが少ない時や
  集計取得に失敗した時は、注目記事・新着記事へ自動でフォールバックする。
