...
`/api/cron/daily-auto-publish`は過去互換のためコードを残しているが、Vercel Cronから外し、
`DAILY_AUTO_PUBLISH_ENABLED=true`が明示されない限りHTTP 410で停止する。通常運用では有効化しない。
収集・AI下書き・画像候補の準備までは自動化し、公開または予約公開は管理画面で内容・画像・出典・
アフィリエイト表記を確認した編集者だけが実行する。
