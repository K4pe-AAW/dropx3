...
test("元情報にリーク・Gossp!の明示があればAI分類より優先できる", () => {
  ...
  assert.equal(detectUnconfirmedStatus("Gossp!｜新作が登場か"), "rumor")
...
test("未確認記事のタイトルに表示が無ければ自動で補う", () => {
  ...
  assert.equal(ensureUnconfirmedTitle("新型モデルが登場", "rumor"), "Gossp!｜新型モデルが登場")
})

test("Gossp!とリークだけを未確認情報として扱う", () => {
