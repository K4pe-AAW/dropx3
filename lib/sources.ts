 ...
 * 企業決算、教育イベント、時事ニュース等も無差別に配信している(2026-08-20に実際に
 * 三宅香帆の読書イベント記事・都築拓紀の書類送検報道・アドビの教育イベント記事や、
 * メイクアップ/香水等のコスメ記事がAI下書きとして紛れ込んだのを機に追加)。
 * ブランド名を網羅するinclusion方式は無数のブランドを列挙できず非現実的なため、
 * 明確に対象外と分かるジャンルだけを弾くexclusion方式にする。lib/collector.ts側で使う。
 */
