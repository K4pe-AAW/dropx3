...
/**
 * FASHIONSNAPのRSSは「ファッション情報サイト」を謳いつつ、実際にはコスメ/美容、
 * 企業決算、教育イベント、時事ニュース、レディース単独の記事等も無差別に配信している
 * (2026-08-20に三宅香帆の読書イベント記事・都築拓紀の書類送検報道・アドビの教育イベント
 * 記事や、メイクアップ/香水等のコスメ記事がAI下書きとして紛れ込んだのを機に除外運用を開始。
 * 2026-08-24、それでも無関係な記事が多く「使いずらい」との判断から、メンズのアパレル系記事
 * とファッションイベント記事だけに絞るexclusion→inclusion方式へ変更)。
 * ブランド名を網羅するinclusion方式は無数のブランドを列挙できず非現実的なため、
 * 「アパレル品目・メンズ・イベントを表す語を含むか」で判定する。lib/collector.ts側で使う。
 */
export const FASHIONSNAP_INCLUDE_KEYWORDS = [
  // メンズ明示
  "メンズ",
  "MEN'S",
  "Men's",
  // アパレル品目(コラボ・発売記事のタイトルに頻出)
  "ジャケット",
  "ブルゾン",
  "コート",
  "ダウン",
...
export const FASHIONSNAP_EXCLUDE_KEYWORDS = [
  "コスメ",
  "コフレ",
  "ビューティ",
  "エイジングケア",
