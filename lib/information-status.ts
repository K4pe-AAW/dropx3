...
export function detectUnconfirmedStatus(text: string): "rumor" | "leak" | undefined {
  ...
  if (/(?:噂|Gossp!|未確認情報|\brumou?rs?\b)/i.test(text)) return "rumor"
  return undefined
}
...
export function ensureUnconfirmedTitle(title: string, status: InformationStatus): string {
  ...
  if (status === "rumor" && !/(?:Gossp!|噂|RUMOR|〜か|登場か)/i.test(title)) return `Gossp!｜${title}`
  return title
}
...
export function unconfirmedNotice(status: "rumor" | "leak"): string {
  return status === "leak"
    ...
    : "本記事の内容は、現時点ではGossp!・未確認情報です。価格、発売日、仕様、国内展開などは変更される可能性があります。今後のブランド公式発表を待ちましょう。"
}
