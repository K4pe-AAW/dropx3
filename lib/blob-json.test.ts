...
test("Blob originの本文と同じ応答のETagを組にして返す", async () => {
  ...
  const result = await readBlobJsonWithEtag<typeof payload>("data/articles.json", async (...args) => {
    calls.push(args)
    return { statusCode: 200, stream, blob: { etag: 'W/"latest-etag"' } }
  })
...
test("Blobが存在しない場合はnullを返す", async () => {
  const result = await readBlobJsonWithEtag("data/missing.json", async () => null)
