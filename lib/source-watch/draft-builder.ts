...
絶対に守るルール:
- 情報源が海外メディア・英語(Grailed等)や他の外国語であっても、title/excerpt/bodyParagraphsは
  必ず完全に日本語で書くこと。商品名・モデル名・カラー名等の固有名詞はそのまま表記してよいが、
  それ以外の英語の単語・文をそのまま残したり、原文の一文をそのまま埋め込んだりしないこと。
...
- 確度がRUMORの場合はタイトルと本文でGoss!p・未確認情報だと明示し、断定表現を避けること。
